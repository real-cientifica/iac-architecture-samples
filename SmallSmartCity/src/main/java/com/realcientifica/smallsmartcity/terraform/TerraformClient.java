...
public class TerraformClient implements AutoCloseable {
    ...
    private static final String VM_SHAPE = "vm_shape", API_FINGERPRINT = "api_fingerprint", API_PRIVATE_KEY_PATH = "api_private_key_path", TENANCY_ID = "tenancy_id", USER_ID = "user_id", REGION = "region", COMPARTMENT_ID = "compartment_id", SSH_PUBLIC_KEY = "ssh_public_key";
    ...
    static {
       ...
       // NON_INTERACTIVE_COMMAND_MAP.put(PLAN_COMMAND,"-var=vm_shape=VM.Standard2.2");
    }
    ...
    private ProcessLauncher getTerraformLauncher(String command) throws IOException {
        ...
        System.out.println("%s"+ TERRAFORM_EXE_NAME + command);
        ...
        /* 
        launcher.setEnvironmentVariable(TENANCY_ID, this.options.getTenancy_id());
        launcher.setEnvironmentVariable(USER_ID, this.options.getUser_id());
        launcher.setEnvironmentVariable(COMPARTMENT_ID, this.options.getCompartment_id());
        launcher.setEnvironmentVariable(SSH_PUBLIC_KEY, this.options.getSsh_public_key());
        launcher.setEnvironmentVariable(API_PRIVATE_KEY_PATH, this.options.getApi_private_key_path());
        launcher.setEnvironmentVariable(API_FINGERPRINT, this.options.getApi_fingerprint());
        launcher.setEnvironmentVariable(VM_SHAPE, this.options.getVm_shape());  
        */ 
        
        launcher.setEnvironmentVariable(REGION, this.options.getRegion());    
        
        /*
        if (!options.equals("")){
           launcher.appendCommands(options);
        }
        */
